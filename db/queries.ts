...
export const getCourseProgress = cache(async () => {
  ...
  if (!userId || !userProgres?.activeCourseId) return null;
  const unitsInActiveCourse = await db.query.units.findMany({
    orderBy: (units, { asc }) => [asc(units.order)],
    where: eq(units.courseId, userProgres.activeCourseId),
    with: {
      lessons: {
        orderBy: (lessons, { asc }) => [asc(lessons.order)],
        with: {
...
export const getLesson = cache(async (id?: number) => {
  const { userId } = await auth();
  if (!userId) return null;
  const cousreProgress = await getCourseProgress();

  const lessonId = id || cousreProgress?.activeLessonId;
...
export const getLessonPercentage = cache(async (id?: number) => {
  const cousreProgress = await getCourseProgress();
  if (!cousreProgress?.activeLessonId) return 0;
  const lesson = await getLesson(cousreProgress.activeLessonId);
  if (!lesson) return 0;
...
export const getUserSubscriptions = cache(
  async (): Promise<
    (typeof userSubscriptions.$inferSelect & { isActive: boolean }) | null
  > => {
    const { userId } = await auth();
    if (!userId) return null;
